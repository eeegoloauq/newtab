...
func TestIconIsServedAsUntrustedBytes(t *testing.T) {
	...
	rec := get(t, New(testPage(t, dir), nil), "/icon/git")
	if rec.Code != http.StatusOK {
...
func TestHealthzOK(t *testing.T) {
	rec := get(t, New(testPage(t, ""), nil), "/healthz")
	if rec.Code != http.StatusOK {
		t.Fatalf("status %d", rec.Code)
	}
...
func TestFaviconIsSilent(t *testing.T) {
	rec := get(t, New(testPage(t, ""), nil), "/favicon.ico")
...
func TestUnknownPathIs404(t *testing.T) {
	rec := get(t, New(testPage(t, ""), nil), "/nope")
